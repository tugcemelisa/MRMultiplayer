...
namespace Chess
{
    ...
    public class Board
    {
        ...
        public event Action<int, int, int> onPieceRemoved;
        public event Action<int, int, int, int> onPieceMoved;

        /// <summary>
        /// Add piece at square, piece type, color
        /// </summary>
        public event Action<int, int, int> onPieceAdded;
        ...
        public void MakeMove(Move move, bool inSearch = false)
        {
            ...
            if (capturedPieceType != 0 && !isEnPassant)
            {
                ...
                onPieceRemoved?.Invoke(moveTo, capturedPieceType, OpponentColour);
            }
            ...
            if (movePieceType == Piece.King)
            {
                ...
                onPieceMoved?.Invoke(moveFrom, moveTo, movePieceType, ColourToMove);
            }
            else
            {
            ...
            if (isPromotion)
            {
                ...
                onPieceRemoved?.Invoke(moveTo, Piece.Pawn, ColourToMove);
                onPieceAdded?.Invoke(moveTo, promoteType, ColourToMove);
            }
            else
            {
                ...
                switch (moveFlag)
                {
                    case Move.Flag.EnPassantCapture:
                        ...
                        onPieceRemoved?.Invoke(epPawnSquare, Piece.Pawn, OpponentColour);
                        break;
                    case Move.Flag.Castling:
                        ...
                        onPieceMoved?.Invoke(castlingRookFromIndex, castlingRookToIndex, Piece.Rook, ColourToMove);
                        break;
                }
            }
        ...
        public void UnmakeMove(Move move, bool inSearch = false)
        {
            ...
            if (movedPieceType == Piece.King)
            {
                ...
                onPieceMoved?.Invoke(movedTo, movedFrom, movedPieceType, ColourToMove);
            }
            else if (!isPromotion)
            {
            ...
            if (isPromotion)
            {
                ...
                onPieceAdded?.Invoke(movedFrom, Piece.Pawn, ColourToMove);
                switch (moveFlags)
                {
                    case Move.Flag.PromoteToQueen:
                        queens[ColourToMoveIndex].RemovePieceAtSquare(movedTo);
                        onPieceRemoved?.Invoke(movedTo, Piece.Queen, ColourToMove);
                        break;
                    case Move.Flag.PromoteToKnight:
                        knights[ColourToMoveIndex].RemovePieceAtSquare(movedTo);
                        onPieceRemoved?.Invoke(movedTo, Piece.Knight, ColourToMove);
                        break;
                    case Move.Flag.PromoteToRook:
                        rooks[ColourToMoveIndex].RemovePieceAtSquare(movedTo);
                        onPieceRemoved?.Invoke(movedTo, Piece.Rook, ColourToMove);
                        break;
                    case Move.Flag.PromoteToBishop:
                        bishops[ColourToMoveIndex].RemovePieceAtSquare(movedTo);
                        onPieceRemoved?.Invoke(movedTo, Piece.Bishop, ColourToMove);
                        break;
                }
            }
            else if (isEnPassant)
            { // ep cature: put captured pawn back on right square
                ...
                onPieceAdded?.Invoke(epIndex, Piece.Pawn, OpponentColour);
            }
            ...
            { // castles: move rook back to starting square
                ...
                ZobristKey ^= Zobrist.piecesArray[Piece.Rook, ColourToMoveIndex, castlingRookFromIndex];
                ZobristKey ^= Zobrist.piecesArray[Piece.Rook, ColourToMoveIndex, castlingRookToIndex];
                onPieceMoved?.Invoke(castlingRookToIndex, castlingRookFromIndex, Piece.Rook, ColourToMove);
            }

            // add back piece that could have been killed from a move. This has to be done here, rather
            // then up above because the prior moves need to first be done undo before you add the piece back.
            if (capturedPieceType != 0 && !isEnPassant)
            {
                onPieceAdded?.Invoke(movedTo, capturedPieceType, OpponentColour);
            }
